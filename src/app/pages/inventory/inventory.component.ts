...
export class InventoryComponent {
  config: ResourceConfig = {
    ...
    summaries: [
      { label: 'Total Items', value: (items) => items.length },
      { label: 'Low Stock', value: (items) => items.filter((item) => item['status'] === 'low-stock').length },
      { label: 'Out of Stock', value: (items) => items.filter((item) => item['status'] === 'out-of-stock').length },
      { label: 'Stock Value', value: (items) => `$${items.reduce((sum, item) => sum + Number(item['quantity'] || 0) * Number(item['unitCost'] || 0), 0).toLocaleString()}` }
    ]
  };
}
